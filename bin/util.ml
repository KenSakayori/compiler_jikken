...
module Option = struct
  ...
  let try_not_found f x = try Some (f x) with Not_found -> None
...
module Files = struct
  let rec remove_rec filename =
    Format.printf "remove: %s@." filename;
  ...
  let rec find ?(filename=".") postfix =
    if Sys.is_directory filename then
      Sys.readdir filename
      |> Array.to_list
      |> List.find_map_opt (fun s -> find ~filename:(filename ^ "/" ^ s) postfix)
    else if String.ends_with filename ~suffix:postfix then
      Some filename
    else
      None
end
...
module Memoize = struct
  let create_gen size =
    ...
    fun wrap x proj f ->
      match Hashtbl.find_option tbl x with
      | Some x -> wrap x
      | None ->
          f x
          |@> Hashtbl.add tbl x -| proj

  let create size =
    let cache = create_gen size in
    fun x f -> cache Fun.id x Fun.id f
end
